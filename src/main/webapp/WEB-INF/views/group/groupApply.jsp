	...
	<div class="container">
		<div class="container-btn">
			...
			<div class="row tableinfo">
							...
							<c:if test="${map.MemberDTO.size() > 0}">
								<c:forEach items="${map.MemberDTO}" var="list">
									<tr>
										...
										<td>${subDate}</td>
										<td><input class="form-check-input checkUser_email" type="checkbox" name="checkUser_email" value="${list.user_email}"></td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
				...
				<!-- 그룹번호 숨겨서 받기 -->
				<div class="d-none">
					<input type="text" value="${seq_group}" name="seq_group" id="seq_group"> <input type="text" value="${count}" name="group_people" id="group_people"> <input type="text" value="${groupCount}" name="groupCount" id="groupCount">
				</div>
				<div class="row">
...
<script>
	...
	$("#groupMember").on(
			"click",
			function() {
				location.href = "/group/toGroupMember?seq_group="
						+ $("#seq_group").val();
			})
	// 모임가입 이동
	$("#groupApply").on(
			"click",
			function() {
				location.href = "/group/toGroupApply?seq_group="
						+ $("#seq_group").val();
			})
	// 모임수정 이동
	$("#groupModify").on(
			"click",
			function() {
				location.href = "/group/toModifyGroup?seq_group="
						+ $("#seq_group").val();
			})
	// 모임해산 이동
	$("#groupDelete").on(
			"click",
			function() {
				location.href = "/group/toGroupDelete?seq_group="
						+ $("#seq_group").val();
			})
	...
	$("#applyBtn").on(
			...
			function() {
				...
				if (checkBoxArr.length < 1) {
					...
					$.ajax({
						url : "/group/completeApply?&group_people=" + count
