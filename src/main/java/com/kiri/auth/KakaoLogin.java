...
public class KakaoLogin {
	...
	private final static String kakaoRedirectUri = "http://localhost:8080/auth/kakaoCallback";
	...
	public String getAuthorizationUrl(HttpSession session) {
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(kakaoClientId).apiSecret(kakaoClientSecret).callback(kakaoRedirectUri).state(state).build(KakaoLoginApi.instance());
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws Exception {
		OAuth20Service oauthService = new ServiceBuilder().apiKey(kakaoClientId).apiSecret(kakaoClientSecret).callback(kakaoRedirectUri).build(KakaoLoginApi.instance());
