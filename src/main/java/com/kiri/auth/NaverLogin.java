...
public class NaverLogin {
	...
	private final static String naverRedirectUri = "http://localhost:8080/auth/naverCallback";
	...
	public String getAuthorizationUrl(HttpSession session) {
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(naverClientId).apiSecret(naverClientSecret).callback(naverRedirectUri).state(state).build(NaverLoginApi.instance());
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		OAuth20Service oauthService = new ServiceBuilder().apiKey(naverClientId).apiSecret(naverClientSecret).callback(naverRedirectUri).build(NaverLoginApi.instance());
